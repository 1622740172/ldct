...
create_left_table <- function(plot_df) {
  ...
  table_data <- data.frame(
    Group = character(n),
    Screened = character(n),
    Unscreened = character(n),
    RR_CI = character(n),
    P = character(n),
    stringsAsFactors = FALSE
  )
  ...
  for (i in 1:n) {
    if (plot_df$type[i] == "header") {
      ...
      table_data$Screened[i] <- ""
      table_data$Unscreened[i] <- ""
      table_data$RR_CI[i] <- ""
      table_data$P[i] <- ""
    } else {
      table_data$Group[i] <- plot_df$display_label[i]
      ...
      if (!is.na(es) && !is.na(ns)) {
        table_data$Screened[i] <- sprintf("%d/%d", es, ns)
      } else {
        table_data$Screened[i] <- "N/A"
      }
      
      if (!is.na(eu) && !is.na(nu)) {
        table_data$Unscreened[i] <- sprintf("%d/%d", eu, nu)
      } else {
        table_data$Unscreened[i] <- "N/A"
      }
  ...
  tg <- tableGrob(
    ...
    theme = ttheme_minimal(
      core = list(
        fg_params = list(fontsize = 8, fontface = "plain", col = "black", lineheight = 0.85),
        bg_params = list(fill = "white", col = NA),
        padding = unit(c(1.5, 3), "mm")
      ),
      colhead = list(
        fg_params = list(fontsize = 8.5, fontface = "bold", col = "black"),
    ...
    if (plot_df$type[i] == "header") {
      for (j in 1:5) {
        ...
        if (idx <= length(tg$grobs)) {
          tg$grobs[[idx]]$gp <- gpar(fontface = "bold", fontsize = 8.5, col = "black")
        }
      }
    }
  }
...
# 保存为TIFF
tiff("/workspace/forest_plot_final_v7.tiff", width = 32, height = 16, units = "in", res = 300, compression = "lzw")
grid.draw(final_plot)
dev.off()

png("/workspace/forest_plot_final_v7.png", width = 3200, height = 1600, res = 150)
grid.draw(final_plot)
dev.off()

cat("\n图片已保存:\n")
cat("- /workspace/forest_plot_final_v7.tiff (300 dpi, LZW压缩)\n")
cat("- /workspace/forest_plot_final_v7.png (预览)\n")
...
fwrite(result_table, "/workspace/rr_results_final_v7.csv")
cat("结果表格已保存到 /workspace/rr_results_final_v7.csv\n")
