...
# 创建标签：分组标题用粗体显示在单独行
plot_data[, label_main := ""]
plot_data[, label_sub := ""]

plot_data[subgroup == "Overall", label_sub := "Overall"]
plot_data[is_header == TRUE & subgroup != "Overall", label_main := group]
plot_data[is_header == TRUE & subgroup != "Overall", label_sub := paste0("  ", subgroup)]
plot_data[is_header == FALSE, label_sub := paste0("  ", subgroup)]
...
create_forest_plot <- function(dt, outcome_name, color) {
  ...
  p <- ggplot() +
    ...
    scale_y_continuous(breaks = dt_sub$y_pos, labels = y_labels) +
