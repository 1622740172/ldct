...
# ============================================
subgroups <- list(
  list(name = "Overall", var = NULL, val = NULL, label = "Overall", is_header = FALSE, indent = 0),
  list(name = "Sex", var = "gender", val = "M", label = "Male", is_header = TRUE, indent = 0),
  list(name = "Sex", var = "gender", val = "F", label = "Female", is_header = FALSE, indent = 1),
  list(name = "Age group", var = "age_group", val = 1, label = "40-49", is_header = TRUE, indent = 0),
  list(name = "Age group", var = "age_group", val = 2, label = "50-54", is_header = FALSE, indent = 1),
  list(name = "Age group", var = "age_group", val = 3, label = "55-74", is_header = FALSE, indent = 1),
  list(name = "Family history", var = "yijijiazushi", val = 0, label = "No", is_header = TRUE, indent = 0),
  list(name = "Family history", var = "yijijiazushi", val = 1, label = "Yes", is_header = FALSE, indent = 1),
  list(name = "Occupational hazard", var = "as_occupational_hazard_exposure", val = 0, label = "No", is_header = TRUE, indent = 0),
  list(name = "Occupational hazard", var = "as_occupational_hazard_exposure", val = 1, label = "Yes", is_header = FALSE, indent = 1),
  list(name = "Chronic resp. disease", var = "as_chronic_respiratory_disease", val = 0, label = "No", is_header = TRUE, indent = 0),
  list(name = "Chronic resp. disease", var = "as_chronic_respiratory_disease", val = 1, label = "Yes", is_header = FALSE, indent = 1)
)
...
plot_data[, y_pos := seq_len(.N), by = outcome]
...
plot_data[indent == 1, display_label := paste0("    ", subgroup)]

# 标记分组标题行
plot_data[, is_group_title := FALSE]
plot_data[is_header == TRUE & subgroup != "Overall", is_group_title := TRUE]
...
create_forest_plot <- function(dt, outcome_name, color) {
  ...
  # 为分组标题添加加粗效果
  label_faces <- ifelse(dt_sub$is_group_title, "bold", "plain")
  ...
  p <- ggplot() +
    ...
    scale_y_continuous(breaks = dt_sub$y_pos, labels = dt_sub$display_label) +
    ...
    theme(
      ...
      axis.text.y = element_text(size = 9.5, lineheight = 0.9),
  ...
  p <- p + annotate("text", x = 3.5, y = dt_sub$y_pos, 
                    label = dt_sub$full_text, 
                    hjust = 0, vjust = 0.5, size = 3.2, color = color, fontface = "plain")
...
# 保存为TIFF
tiff("/workspace/forest_plot_R.tiff", width = 26, height = 16, units = "in", res = 300, compression = "lzw")
grid.draw(combined)
dev.off()

png("/workspace/forest_plot_R.png", width = 2600, height = 1600, res = 150)
grid.draw(combined)
dev.off()
...
cat("结果表格已保存到 /workspace/rr_results_table.csv\n")
