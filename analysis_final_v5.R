...
for (outcome in names(outcomes)) {
  ...
  events_s_col <- paste0("events_s_", outcome)
  events_u_col <- paste0("events_u_", outcome)
  ...
  plot_df[[p_col]] <- NA
  plot_df[[events_s_col]] <- NA
  plot_df[[events_u_col]] <- NA
  plot_df[[n_s_col]] <- NA
  plot_df[[n_u_col]] <- NA
  ...
  for (i in 1:n) {
    ...
    if (res$type == "data") {
      ...
      plot_df[[events_s_col]][i] <- res$events_screened
      plot_df[[events_u_col]][i] <- res$events_unscreened
...
create_left_table <- function(plot_df) {
  ...
  table_data <- data.frame(
    Group = character(n),
    Events_Screened = character(n),
    Events_Unscreened = character(n),
    RR_CI = character(n),
    P = character(n),
    stringsAsFactors = FALSE
  )
  ...
  for (i in 1:n) {
    if (plot_df$type[i] == "header") {
      ...
      table_data$Events_Screened[i] <- ""
      table_data$Events_Unscreened[i] <- ""
      table_data$RR_CI[i] <- ""
      table_data$P[i] <- ""
    } else {
      table_data$Group[i] <- plot_df$display_label[i]
      
      # 使用LC_event的事件数（三个结局共用）
      es <- plot_df$events_s_LC_event[i]
      eu <- plot_df$events_u_LC_event[i]
      ns <- plot_df$n_s_LC_event[i]
      nu <- plot_df$n_u_LC_event[i]
      
      if (!is.na(es) && !is.na(ns)) {
        table_data$Events_Screened[i] <- sprintf("%d/%d", es, ns)
      } else {
        table_data$Events_Screened[i] <- "N/A"
      }
      
      if (!is.na(eu) && !is.na(nu)) {
        table_data$Events_Unscreened[i] <- sprintf("%d/%d", eu, nu)
      } else {
        table_data$Events_Unscreened[i] <- "N/A"
      }
      
      # RR和CI - 使用Overall的RR（这里简化，实际应该显示三个结局的RR）
      rr <- plot_df$rr_LC_event[i]
      ci_l <- plot_df$ci_lower_LC_event[i]
      ci_u <- plot_df$ci_upper_LC_event[i]
      
      if (!is.na(rr) && !is.infinite(rr)) {
        table_data$RR_CI[i] <- sprintf("%.2f (%.2f-%.2f)", rr, ci_l, ci_u)
      } else {
        table_data$RR_CI[i] <- "N/A"
      }
      
      p <- plot_df$p_LC_event[i]
      if (!is.na(p)) {
        if (p < 0.0001) {
          table_data$P[i] <- "<0.0001"
        } else {
          table_data$P[i] <- sprintf("%.4f", p)
        }
      } else {
        table_data$P[i] <- "N/A"
      }
  ...
  tg <- tableGrob(
    d = table_data,
    rows = NULL,
    cols = c("Subgroup", "Screened\nEvents/N", "Unscreened\nEvents/N", "RR (95% CI)", "P"),
    theme = ttheme_minimal(
      core = list(
        fg_params = list(fontsize = 8.5, fontface = "plain", col = "black", lineheight = 0.9),
        bg_params = list(fill = "white", col = NA),
        padding = unit(c(2, 4), "mm")
      ),
      colhead = list(
        fg_params = list(fontsize = 9, fontface = "bold", col = "black"),
        bg_params = list(fill = "gray95", col = "gray80"),
  ...
  # 设置列宽
  tg$widths <- unit(c(0.22, 0.16, 0.16, 0.28, 0.18), "npc")
    ...
    if (plot_df$type[i] == "header") {
      for (j in 1:5) {
        ...
        if (idx <= length(tg$grobs)) {
          tg$grobs[[idx]]$gp <- gpar(fontface = "bold", fontsize = 9, col = "black")
        }
      }
    }
  }
...
create_forest_plot <- function(plot_df, outcome_name) {
  ...
  p <- ggplot() +
    geom_segment(data = plot_df_valid,
                 aes_string(x = ci_lower_col, xend = ci_upper_col, y = "y_pos", yend = "y_pos"),
                 color = color, linewidth = 1.5, alpha = 0.7) +
    geom_point(data = plot_df_valid,
               aes_string(x = rr_col, y = "y_pos"),
...
# 保存为TIFF
tiff("/workspace/forest_plot_final_v5.tiff", width = 32, height = 16, units = "in", res = 300, compression = "lzw")
grid.draw(final_plot)
dev.off()

png("/workspace/forest_plot_final_v5.png", width = 3200, height = 1600, res = 150)
grid.draw(final_plot)
dev.off()

cat("\n图片已保存:\n")
cat("- /workspace/forest_plot_final_v5.tiff (300 dpi, LZW压缩)\n")
cat("- /workspace/forest_plot_final_v5.png (预览)\n")
  ...
  for (res in all_results[[outcome]]) {
    if (res$type == "data") {
      result_table <- rbind(result_table, data.frame(
        ...
        Screened_PY = round(res$py_screened, 1),
        Unscreened_N = res$n_unscreened,
        Unscreened_Events = res$events_unscreened,
        Unscreened_PY = round(res$py_unscreened, 1),
...
fwrite(result_table, "/workspace/rr_results_final_v5.csv")
cat("结果表格已保存到 /workspace/rr_results_final_v5.csv\n")
