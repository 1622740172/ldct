...
create_forest_plot <- function(dt_list, outcome_name) {
  ...
  p <- ggplot() +
    ...
    theme(
      ...
      plot.margin = margin(10, 200, 10, 10)
    )
  
  # 添加RR(95%CI)数值标签
  p <- p + annotate("text", x = 3.2, y = plot_df$y_pos,
  ...
  # 添加P值标签
  p <- p + annotate("text", x = 4.6, y = plot_df$y_pos,
...
# 保存为TIFF
tiff("/workspace/forest_plot_final_v3.tiff", width = 28, height = 16, units = "in", res = 300, compression = "lzw")
grid.draw(final_combined)
dev.off()

png("/workspace/forest_plot_final_v3.png", width = 2800, height = 1600, res = 150)
grid.draw(final_combined)
dev.off()

cat("\n图片已保存:\n")
cat("- /workspace/forest_plot_final_v3.tiff (300 dpi, LZW压缩)\n")
cat("- /workspace/forest_plot_final_v3.png (预览)\n")
...
fwrite(result_table, "/workspace/rr_results_final_v3.csv")
cat("结果表格已保存到 /workspace/rr_results_final_v3.csv\n")
