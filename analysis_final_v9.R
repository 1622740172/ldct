...
# 导出表格为PNG
tiff("/workspace/table_subgroup.tiff", width = 10, height = 12, units = "in", res = 300, compression = "lzw")
grid.draw(tg)
dev.off()

png("/workspace/table_subgroup.png", width = 1000, height = 1200, res = 150)
grid.draw(tg)
dev.off()

cat("\n表格已导出:\n")
cat("- /workspace/table_subgroup.tiff (300 dpi, LZW压缩)\n")
cat("- /workspace/table_subgroup.png (预览)\n")
...
# 导出森林图为TIFF
tiff("/workspace/forest_plot_subgroup.tiff", width = 18, height = 12, units = "in", res = 300, compression = "lzw")
grid.draw(forest_combined)
dev.off()

png("/workspace/forest_plot_subgroup.png", width = 1800, height = 1200, res = 150)
grid.draw(forest_combined)
dev.off()
...
tiff("/workspace/combined_table_forest.tiff", width = 28, height = 14, units = "in", res = 300, compression = "lzw")
grid.draw(final_plot)
dev.off()

png("/workspace/combined_table_forest.png", width = 2800, height = 1400, res = 150)
grid.draw(final_plot)
dev.off()

cat("\n组合图已导出:\n")
cat("- /workspace/combined_table_forest.tiff (300 dpi, LZW压缩)\n")
cat("- /workspace/combined_table_forest.png (预览)\n")
...
fwrite(result_table, "/workspace/rr_results_final_v9.csv")
cat("\n结果表格已保存到 /workspace/rr_results_final_v9.csv\n")
